...
function HomeTab() {
  ...
  const handleSave = async () => { if (!editingId) return; await supabase.from('home_sections').update(editForm).eq('id', editingId); setEditingId(null); };
  const handleTogglePublish = async (id: string, current: boolean) => { await supabase.from('home_sections').update({ is_published: !current }).eq('id', id); };
              ...
              <tr key={section.id} className="border-b">
                ...
                <td className="py-2">
                  {editingId === section.id ? (
                    <input type="text" value={editForm.title || ''} onChange={e => setEditForm({ ...editForm, title: e.target.value })} className="border rounded px-2 py-1" />
                  ) : section.title}
                </td>
                <td className="py-2">
                ...
                <td className="py-2">
                  {editingId === section.id ? (
                    <><button onClick={handleSave} className="text-green-600 mr-2">Save</button><button onClick={() => setEditingId(null)} className="text-gray-500">Cancel</button></>
...
function DataTable({ data, columns, tableName }: { data: any[]; columns: string[]; tableName: string }) {
  ...
  const handleDelete = async (id: string) => { if (!confirm('Delete?')) return; await supabase.from(tableName).delete().eq('id', id); };
  const handleTogglePublish = async (id: string, current: boolean) => { await supabase.from(tableName).update({ is_published: !current }).eq('id', id); };

  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead><tr className="border-b">{columns.map(col => <th key={col} className="py-2 text-left capitalize">{col.replace('_',' ')}</th>)}<th className="py-2 text-left">Published</th><th className="py-2 text-left">Actions</th></tr></thead>
        <tbody>
          {data.map(row => (
            <tr key={row.id} className="border-b">
              {columns.map(col => (
                <td key={col} className="py-2">
                  {editingId === row.id ? (
                    <input type={col.includes('price') ? 'number' : 'text'} value={editForm[col] || ''} onChange={e => setEditForm({ ...editForm, [col]: e.target.value })} className="border rounded px-2 py-1 w-full" />
                  ) : String(row[col] ?? '—')}
                </td>
              ))}
              <td className="py-2">
                <button onClick={() => handleTogglePublish(row.id, row.is_published)} className={`px-2 py-1 rounded text-xs ${row.is_published ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
              ...
              <td className="py-2">
                {editingId === row.id ? (
                  ...
                  <><button onClick={() => handleEdit(row)} className="text-prakash-gold mr-2">Edit</button><button onClick={() => handleDelete(row.id)} className="text-red-500">Del</button></>
                )}
              </td>
            </tr>
          ))}
...
/* ------------------------------------------------------------------ */
/* Individual Tabs using DataTable                                      */
/* ------------------------------------------------------------------ */
function FreeToolsTab() { const { items, loading } = useRealtimeContent<FreeTool>('free_tools', 'order_index'); if (loading) return <LoadingSpinner />; return <DataTable data={items} columns={['tool_key','title','description','premium_price','is_published']} tableName="free_tools" />; }
function ServicesTab() { const { items, loading } = useRealtimeContent<Service>('services', 'order_index'); if (loading) return <LoadingSpinner />; return <DataTable data={items} columns={['slug','title','description','is_published']} tableName="services" />; }
function CoursesTab() { const { items, loading } = useRealtimeContent<Course>('courses', 'created_at', false); if (loading) return <LoadingSpinner />; return <DataTable data={items} columns={['slug','title','level','subject','price','is_published']} tableName="courses" />; }
function BlogsTab() { const { items, loading } = useRealtimeContent<BlogPost>('blog_posts', 'published_at', false); if (loading) return <LoadingSpinner />; return <DataTable data={items} columns={['slug','title','category','is_published']} tableName="blog_posts" />; }
function TestimonialsTab() { const { items, loading } = useRealtimeContent<Testimonial>('testimonials', 'order_index'); if (loading) return <LoadingSpinner />; return <DataTable data={items} columns={['client_name','location','rating','is_published']} tableName="testimonials" />; }
...
function AIToolsTab() {
  ...
  useEffect(() => { supabase.rpc('get_tool_usage_stats').then(({ data }) => { setStats(data || []); setLoading(false); }); }, []);
  if (loading) return <LoadingSpinner />;
  return (
